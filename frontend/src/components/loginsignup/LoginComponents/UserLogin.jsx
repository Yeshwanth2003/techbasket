...
import { loginUserRoute } from "../../utilityFiles/urlMap";
...
export default function UserLogin({ customNavigation }) {
  ...
  function onSubmit(event) {
    ...
    // onFormSuccess
    function onFormSuccess(err, data) {
      if (err) return;
      exportFetch(loginUserRoute, data).then((resData) => {
        console.log(resData);
        
        if (!resData.exist) {
          alertBoxRef.current.toggleAlert("Enter Valid Details");
          return;
        }
        // if present in table set reducer and route
    ...
    formExtractor(udata, event.target, alertBoxRef, onFormSuccess);
  }
  return (
    <>
